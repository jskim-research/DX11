...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	// device, device context => interface to all of the Direct3D functions
	// Create the swap chain, Direct3D device, and Direct3D device context.
	// DirectX 11 지원 GPU 가 아닌 경우 D3D_DRIVER_TYPE_REFERENCE 를 통해 CPU 로 그리기 가능 (다만 속도 1/1000 수준)
	...
	// depth buffer -> polygon 3D scene 에 적절하게 렌더링 될 수 있도록
	// attach stencil buffer to depth buffer -> motion blur, volumetric shadows 등에 사용
	...
	// view matrix (camera) => 나중에 따로 camera 클래스로 뺄 것
	// Create an orthographic projection matrix for 2D rendering. (UI 등)
