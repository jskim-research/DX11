...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// 컴파일: HLSL 텍스트 코드 -> GPU 기계어 형태의 shader bytecode
	...
	// bytecode 를 기반으로 GPU 용 shader 객체 생성
	...
	polygonLayout[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;  // 알아서 offset 계산하도록 함
...
bool ColorShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	...
	// Map/Unmap 은 CPU 가 GPU 리소스인 matrix buffer 의 데이터를 업데이트하기 위한 절차
	// GPU 는 병렬 처리 중이므로 critical section 발생 가능하기 때문에 조심히 접근
	...
	// vertex shader 코드에서 MatrixBuffer 가 0 번째 slot 에 있음 가정
